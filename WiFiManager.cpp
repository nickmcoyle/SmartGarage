...
boolean WiFiManager::autoConnect(char const *apName, char const *apPassword) {
  ...
  if(getWiFiIsSaved()) {    
    ...
    //attempts to connect to wifi using saved info with connectWifi()
...
void WiFiManager::setupConfigPortal() {
  ...
  //add my own method here for handleUseWithoutWifi
  server->on(String(FPSTR(R_usewithoutwifi)).c_str(), std::bind(&WiFiManager::handleUseWithoutWiFi, this));
...
bool WiFiManager::shutdownConfigPortal(){
  ...
  server->stop();
  server.reset();
  //server->close(); //I added this, causes panic reboot, but at least it connects to wifi and starts the server successfully after the reboot
...
/**
 * method to allow using the device without WiFi connection
 */
void WiFiManager::handleUseWithoutWiFi() {  
  settings::set_use_without_wifi(true);
