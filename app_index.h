...
<html>
    ...
    <body>
  <!-- WiFi enabled espresso PID webpage -->    
    <header>  
    <div class="navbar">
      <h1><i class="fa fa-coffee"></i>&nbsp;<span id="app-name"></span></h1>   
        <div class="dropdown">
  ...
  <div id='wrapper'>             
   <div id='success-message' class='alert'>
    ...
    <div id='info-wrapper'>
      <div class='container-info' id='power-button-display'>
        <label>POWER</label>                
        //<input type='submit' id='power-status' value='<i class="fa fa-power-off" style="font-size:24px"></i>' class='rounded grey'>
        <a id='power-status' value='OFF' class='rounded grey'></a>
      </div>        
          
      <div class='container-info' id='temperature-display'>
        <label>TEMP</label>           
        <a id='temperature' class='rounded green'></a>
      </div>          
      
      <div class='container-info' id='power-status-display'>
        <label>RUN TIME</label>           
        <a id='run-time' class='rounded blue default-action'>--</a>
      </div>
     </div><!-- end info wrapper-->
     
     <div id='input-wrapper'> 
     <ul class='input-list' id='pid-value-form'>
      <li>
        <label for='targetTemp'>Target Temp</label>
        <input class='input-rounded pid-value' id='target-temperature' type='number' pattern='\d*' min=0 step=1 name='targettemp'>
      </li>
      <li>
        <label for='pgain'>Pgain</label>
        <input class='input-rounded pid-value' id='p-gain' type='number' pattern='\d*' min=0 step=1 name='pgain'>
      </li>
       <li>
        <label for='igain'>Igain</label>
        <input class='input-rounded pid-value' id='i-gain' type='number' pattern='\d*' min=0 step=1  name='igain'>
      </li>
       <li>
        <label for='dgain'>Dgain</label>
        <input class='input-rounded pid-value' id='d-gain' type='number' pattern='\d*' min=0 step=1 name='dgain'>
      </li>
       <li>
        <label for='windupGuard'>Windup Guard</label>
        <input class='input-rounded pid-value' id='windup-guard-gain' type='number' pattern='\d*' min=0 step=1 name='windupguard'>
      </li>       
      <li>              
    ...
    <script type="text/javascript">            
      document.addEventListener('DOMContentLoaded', function (event) {
        ...
        var getPIDValues = function() {
          // read values from server
          var xhr = new XMLHttpRequest();
          xhr.open('GET', document.location.origin + '/get-pid-values', true);
          xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
          xhr.onload = function () {
            var json = JSON.parse(this.responseText);                                  
            for(var key in json) {          
        ...
        window.getTemperature = function() {
          // read values from server
          var xhr = new XMLHttpRequest();        
          xhr.open('GET', document.location.origin + '/get-temperature', true);
          xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
          xhr.onload = function () {          
            var json = JSON.parse(this.responseText);                                  
            for(var key in json) {          
        ...
        document.getElementById('submitBtn').addEventListener('click', function () {
          var formData = document.getElementById('pid-value-form').getElementsByTagName('input');
          ...
          var xhr = new XMLHttpRequest();
          xhr.open('POST', document.location.origin + '/set-pid-values', true);
          xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        ...
        document.getElementById('defaultsBtn').addEventListener('click', function () { 
          ...
          var xhr = new XMLHttpRequest();
          xhr.open('POST', document.location.origin + '/set-default-pid-values', true);
          xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
          xhr.onload = function () {
            var json = JSON.parse(this.responseText);                  
            for(var key in json) {            
        ...
        document.getElementById('power-status').addEventListener('click', function () {      
          // read values from server
          var xhr = new XMLHttpRequest();
          xhr.open('POST', document.location.origin + '/toggle-power', true);
          xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
          xhr.onload = function () {
            var json = JSON.parse(this.responseText);
            for(var key in json) {          
